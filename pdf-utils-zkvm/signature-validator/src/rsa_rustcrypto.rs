...
impl PublicKey {
    pub fn from_components(n: &[u8], e: &[u8]) -> Result<Self, alloc::string::String> {
        ...
        debug_log!("RSA from_components: modulus len={}, exponent len={}", n.len(), e.len());
        ...
        let n_boxed = BoxedUint::from_be_slice(n, n_bits as u32)
            .map_err(|_e| {
        ...
        let e_boxed = BoxedUint::from_be_slice(e, n_bits as u32)
            .map_err(|_e| {
        ...
        let inner = RsaPublicKey::new(n_boxed, e_boxed)
            .map_err(|_e| {
    ...
    pub fn verify_pkcs1v15(
        ...
        debug_log!("DigestInfo length: {}, prefix: {} bytes, hash: {} bytes", 
                   digest_info.len(), prefix.len(), hashed.len());
