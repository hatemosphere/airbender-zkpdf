...
unsafe impl GlobalAlloc for SimpleAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        heap_ref.allocate_first_fit(layout)
            .ok()
...
core::arch::global_asm!(include_str!("../../airbender/examples/scripts/asm/asm_reduced.S"));
...
unsafe fn workload() -> ! {
    ...
    let _ = write!(uart, "Heap: start={:x}, end={:x}, size={}", heap_start, heap_end, heap_size);
    
    // Debug: return heap info
    if heap_size == 0 || heap_start >= heap_end {
        zksync_os_finish_success(&[0xDEAD0001, heap_start as u32, heap_end as u32, heap_size as u32, 0, 0, 0, 0]);
    }
    ...
    let _ = write!(uart, "PDF size: {} bytes", input_size);
    ...
    if input_size > 10_000_000 || input_size == 0 {
        ...
        zksync_os_finish_success(&[0xFFFFFFFF, 1, input_size_word, input_size as u32, 0, 0, 0, 0]);
    }
    
    // Read PDF data word by word
    let mut pdf_data = Vec::with_capacity(input_size);
    let words_to_read = (input_size + 3) / 4; // Round up to next word
    ...
    let _ = write!(uart, "Read {} bytes of PDF data", pdf_data.len());
    ...
    if pdf_data.len() >= 8 {
        ...
        let _ = write!(uart, "PDF header: {:?}", &pdf_data[0..8]);
        if !header_valid {
            // Bad PDF header
            zksync_os_finish_success(&[0xFFFFFFFF, 5, pdf_data[0] as u32, pdf_data[1] as u32, pdf_data[2] as u32, pdf_data[3] as u32, 0, 0]);
        }
    }
    ...
    let _ = write!(uart, "Expected text size word: 0x{:08x} = {}", expected_text_size_word, expected_text_size);
    
    // Read expected text if provided
    let expected_text = if expected_text_size > 0 {
        let mut text = Vec::with_capacity(expected_text_size);
        let words_to_read = (expected_text_size + 3) / 4;
    ...
    let _ = write!(uart, "Has ByteRange: {}, Has Contents: {}", has_byterange, has_contents);
        ...
        if let Some(br_pos) = pdf_data.windows(10).position(|w| w == b"/ByteRange") {
            ...
            if let Some(bracket_start) = pdf_data[br_pos+10..].iter().position(|&b| b == b'[') {
                ...
                if let Some(bracket_end) = pdf_data[bracket_start..].iter().position(|&b| b == b']') {
                    ...
                    if let Ok(range_str) = core::str::from_utf8(&pdf_data[bracket_start+1..bracket_end]) {
                        ...
                        if parts.len() == 4 {
                            if let (Ok(offset1), Ok(length1), Ok(offset2), Ok(_length2)) = (
                                ...
                                let sig_end = offset2;
                                let _ = write!(uart, "Signature range: {} to {}", sig_start, sig_end);
                                
                                // Look for /Contents in that range
                                if sig_end > sig_start && sig_end <= pdf_data.len() {
                                    let sig_range = &pdf_data[sig_start..sig_end];
                                    let contents_in_range = sig_range.windows(9).any(|w| w == b"/Contents");
                                    let _ = write!(uart, "/Contents in signature range: {}", contents_in_range);
                                    
                                    // Print first 100 bytes of signature range
                                    let preview_len = core::cmp::min(100, sig_range.len());
                                    if let Ok(preview) = core::str::from_utf8(&sig_range[0..preview_len]) {
                                    ...
                                    if br_pos > 100 {
                                        ...
                                        if let Some(contents_pos) = before_range.windows(9).position(|w| w == b"/Contents") {
                                            let _ = write!(uart, "/Contents found {} bytes before /ByteRange", 100 - contents_pos);
                                        }
                                    }
                                }
                            }
    ...
    let result = match pdf_utils_zkvm_core::extract_text(pdf_data.clone()) {
        Ok(text_pages) => {
            let _ = write!(uart, "Text extraction successful! {} pages", text_pages.len());
        ...
        Err(e) => {
            ...
            let error_hash = {
                ...
                for (i, byte) in alloc::format!("{}", e).bytes().enumerate() {
                    if i >= 16 { break; }  // Only hash first 16 bytes
                    hash = hash.rotate_left(3) ^ (byte as u32);
                }
                hash
            };
            zksync_os_finish_success(&[0xFFFFFFFF, 2, error_hash, pdf_data.len() as u32, 0, 0, 0, 0]);
        }
    };
    ...
    let (text_found, page_found) = if let Some(expected_bytes) = expected_text {
        let _ = write!(uart, "Checking for expected text of {} bytes", expected_bytes.len());
        ...
        for (idx, page_text) in result.text_pages.iter().enumerate() {
            let _ = write!(uart, "Page {} text: '{}'", idx, page_text);
            let _ = write!(uart, "  Trimmed: '{}'", page_text.trim());
            let _ = write!(uart, "  Length: {}, bytes: {:?}", page_text.len(), &page_text.as_bytes()[..core::cmp::min(50, page_text.len())]);
    ...
    let _ = write!(uart, "Success! Sig={}, TextFound={}, Page={}, NumPages={}", 
        sig_valid, text_found, page_found, num_pages);
